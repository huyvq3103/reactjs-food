...
import {
  ...
  FETCH_CATEGORY_MEAL_BEGIN,
  FETCH_CATEGORY_MEAL_ERROR,
  FETCH_CATEGORY_MEAL_SUCCESS,
...
export const startFetchMealByCategory = async (dispatch, category) => {
  try {
    dispatch({ type: FETCH_CATEGORY_MEAL_BEGIN });
    const response = await axios.get(`${MEAL_CATEGORIES_URL}${category}`);
    dispatch({
      type: FETCH_CATEGORY_MEAL_SUCCESS,
      payload: response.data.meals,
    });
  } catch (error) {
    dispatch({ type: FETCH_CATEGORY_MEAL_ERROR, payload: error.message });
  }
};
